/*
   NOTICE
   the functionality in this file is by
...
export class Localization {
  ...
  static readonly data: { [key: string]: LocalizationData } = {
    en: {
      directions: {
        N: 'north',
        NE: 'northeast',
      ...
      formatOrder: function (n: number): string {
        const i = (n % 10) - 1;
  ...
  constructor(langs: string | string[]) {
    this._langs = Array.isArray(langs) ? langs.slice() : [langs, 'en'];

    for (let i = 0, l = this._langs.length; i < l; i++) {
      const generalizedCode = /([A-Za-z]+)/.exec(this._langs[i]);
      if (generalizedCode && !Localization.data[this._langs[i]]) {
        if (Localization.data[generalizedCode[1]]) {
          this._langs[i] = generalizedCode[1];
        } else {
          throw new Error(
  ...
  localize(keys: string | string[]): any {
    ...
    for (let i = 0, l = this._langs.length; i < l; i++) {
      dict = Localization.data[this._langs[i]];
...
// Add aliases for Spanish
Localization.data['sp'] = Localization.data['es'];
